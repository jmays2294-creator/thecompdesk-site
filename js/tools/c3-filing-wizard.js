...
(function (window) {
   ...
   * with the app wizard (www/js/dashboard/c3-wizard.js). Verified headless by
   * ops/dev/migrations/../tests c3_3 fill harness (26/26 boxes). */
  var C33 = {
  ...
  /* ---- WCB self-file email (honest mailto: — the claimant sends it) --------
   * A plain mailto cannot carry an attachment, and we must NOT file on the
   * claimant's behalf. So this opens the claimant's OWN mail client, pre-addressed
   * to the WCB with the claim identifiers filled in, and tells them to attach the
   * PDF(s) they already downloaded and press send themselves. The C-3.3 (when it
   * exists) is part of the same packet, so the copy references both forms. */
  var WCB_FILING_EMAIL = 'wcbclaimsfiling@wcb.ny.gov';
  function buildWcbMailto(o) {
    o = o || {};
    var name = o.name || '';
    var hasC3 = o.hasC3 !== false;           // default true
    var hasC33 = !!o.hasC33;
    var pkg;
    if (hasC3 && hasC33) pkg = 'my completed Form C-3 (Employee Claim) and Form C-3.3 (Limited Release of Health Information)';
    else if (hasC33) pkg = 'my completed Form C-3.3 (Limited Release of Health Information)';
    else pkg = 'my completed Form C-3 (Employee Claim)';
    ...
    var body = [
      ...
      'Claimant: ' + name,
      'Date of birth: ' + (o.dob || '—'),
      'Date of injury: ' + (o.doi || '—'),
      'WCB case number: ' + (o.wcb || 'Not yet assigned'),
      '',
  ...
  function render(ctx) {
    ...
    step0.appendChild(el('div', { class: 'info-callout', style: 'margin-top:16px', html: 'Just need the medical-records release? <a href="#" id="c3fw-c33-only-link" style="color:var(--accent-hover);font-weight:700">Complete Form C-3.3 (HIPAA) on its own →</a><br><span style="color:var(--tm)">The C-3.3 authorizes the doctors who treated a previous injury to release those records to the insurer. File it with your C-3, or by itself.</span>' }));
    ...
    var stepC33 = el('div', { class: 'step-section', id: 'c3fw-step-c33' });
    ...
    sc1.appendChild(fieldRow([
      textField('c3fw-c33s-name', 'Full Legal Name', 'req', state.name, 'First MI Last', !!state.name),
      dateField('c3fw-c33s-dob', 'Date of Birth', 'req', state.dob, !!state.dob)
    ]));
    sc1.appendChild(fieldRow([
      textField('c3fw-c33s-ssn', 'Social Security Number', 'opt', state.ssn, 'XXX-XX-XXXX', false),
      dateField('c3fw-c33s-doi', 'Date of Current Injury/Illness', 'opt', state.doi, !!state.doi)
    ]));
    sc1.appendChild(group([el('label', { class: 'form-label', html: 'Mailing Address<span class="opt">(optional)</span>' + prefillTag(!!state.mailing) }), el('input', { type: 'text', class: 'form-input', id: 'c3fw-c33s-mailing', value: state.mailing, placeholder: 'Number and street' })]));
    sc1.appendChild(group([el('label', { class: 'form-label', html: 'City, State, ZIP<span class="opt">(optional)</span>' }), el('input', { type: 'text', class: 'form-input', id: 'c3fw-c33s-mailing2', value: state.mailing2, placeholder: 'City, NY 10001' })]));
    sc1.appendChild(group([el('label', { class: 'form-label', html: 'Current injury/illness (all body parts)<span class="req">*</span>' }), el('textarea', { class: 'form-input', id: 'c3fw-c33s-injury', placeholder: 'e.g. lower back and left hip' }, [state.nature]), errEl('c3fw-err-c33s-injury', 'Describe your current injury/illness')]));
    ...
    sc2.appendChild(group([el('label', { class: 'form-label', html: 'Provider(s) (name &amp; address)<span class="req">*</span>' }), el('textarea', { class: 'form-input', id: 'c3fw-c33s-providers', placeholder: 'Dr. Jane Smith — 1 Main St, Albany NY 12203\nUrgent Care — 9 Market St, Troy NY 12180' }, [state.c33_providers]), errEl('c3fw-err-c33s-providers', 'List at least one provider')]));
    var mhToggleS = el('div', { class: 'toggle-switch' + (state.c33_releaseMentalHealth ? ' on' : ''), id: 'c3fw-c33s-mh-toggle' }, [el('div', { class: 'toggle-knob' })]);
    ...
    sc3.appendChild(group([el('label', { class: 'form-label', text: 'Type your full legal name to certify' }), el('input', { type: 'text', class: 'form-input', id: 'c3fw-c33s-certName', value: state.certName, placeholder: 'Your full legal name' })]));
    var sigCanvasC33 = el('canvas', { class: 'sig-pad', id: 'c3fw-sig-c33' });
    ...
    var certToggleC33 = el('div', { class: 'toggle-switch', id: 'c3fw-c33s-cert-toggle' }, [el('div', { class: 'toggle-knob' })]);
    ...
    stepC33.appendChild(el('div', { class: 'btn-row' }, [
      ...
      el('button', { class: 'btn btn-primary', id: 'c3fw-c33-generate', onclick: function () { generate(certAgreedC33.v); } }, ['Generate Form C-3.3'])
    ]));
    ...
    var C33_FIELDS = [['c3fw-c33s-name', 'name'], ['c3fw-c33s-dob', 'dob'], ['c3fw-c33s-ssn', 'ssn'], ['c3fw-c33s-doi', 'doi'], ['c3fw-c33s-mailing', 'mailing'], ['c3fw-c33s-mailing2', 'mailing2'], ['c3fw-c33s-injury', 'nature'], ['c3fw-c33s-providers', 'c33_providers'], ['c3fw-c33s-certName', 'certName']];
    ...
    function goToStandaloneC33() {
      ...
      root.querySelectorAll('.step-section').forEach(function (s) { s.classList.remove('active'); });
      var sec = $('c3fw-step-c33'); if (sec) sec.classList.add('active');
    ...
    SelfFilePackage.prototype.submit = function (pdfBytes, c33Bytes) {
      ...
      return first
        ...
        .then(function (c33path) {
          ...
          return api.from('c3_filings').insert(row).then(function (res) { if (res && res.error) throw res.error; return { kind: 'self_file', saved: true, path: c3path, c33path: c33path }; });
        })
        .then(function (out) {
          var p = out.path
    ...
    // Anonymous: hand whichever bytes were generated straight back as in-browser
    // object URLs. Nothing is uploaded and no row is written — the PDFs live only
    ...
    DownloadOnlyPackage.prototype.submit = function (pdfBytes, c33Bytes) {
      ...
      if (pdfBytes) out.signedUrl = URL.createObjectURL(new Blob([pdfBytes], { type: 'application/pdf' }));
    ...
    function generate(certAgreed) {
      ...
      working = true;
      var btn = $(c33Only ? 'c3fw-c33-generate' : 'c3fw-generate'); if (btn) { btn.disabled = true; btn.textContent = 'Generating…'; }
    ...
    function showSuccess(result) {
      ...
      var hasC3 = !!result.signedUrl || (result.saved && result.path);
      var c33Only = !hasC3 && !!result.c33path;
      var both = hasC3 && !!result.c33path;
      // attach-instruction noun: BOTH forms when bundled, else the single form
      var pktNoun = both ? 'both PDFs (your C-3 and C-3.3)' : (c33Only ? 'the PDF' : 'the PDF');
      ...
      var savedNote = (result.saved ? 'We generated and saved your signed ' : 'We generated your signed ') + formName + '.';
      var screen = el('div', { class: 'success-screen' }, [
        el('div', { class: 'success-icon', text: '✓' }),
        el('h2', { text: headline }),
        el('p', { text: savedNote + c33Note + c33OnlyNote + ' It has not been submitted to the WCB — here’s how to file it.' })
      ]);
      ...
      // Email-submit (honest): opens the claimant's mail client pre-addressed to
      // the WCB; THEY attach the downloaded PDF(s) and press send. We never send
      // it for them and never claim it was submitted.
      var emailHref = buildWcbMailto({ name: state.name, dob: fmtDate(state.dob), doi: fmtDate(state.doi), wcb: profile.wcb_case_number, hasC3: hasC3, hasC33: !!result.c33path });
      screen.appendChild(el('a', { class: 'btn btn-secondary', href: emailHref, style: 'display:block;text-decoration:none;margin-bottom:6px' }, ['✉️ Email it to the WCB yourself']));
      screen.appendChild(el('div', { class: 'form-hint', style: 'text-align:center;margin:-2px 0 12px', text: 'Opens your email app with the details filled in. Attach ' + pktNoun + ' you downloaded above, then press send — you file it yourself; we don’t send it for you.' }));
      ...
      if (both) steps.appendChild(fstep('!', '<b>File both together.</b> The C-3 and C-3.3 must be submitted as one packet — attach both PDFs to the same email/upload below.'));
      ...
      steps.appendChild(fstep(base + 1, '<b>By email (self-file):</b> attach ' + pktNoun + ' and send to <b>wcbclaimsfiling@wcb.ny.gov</b>.'));
      steps.appendChild(fstep(base + 2, '<b>Online:</b> upload ' + pktNoun + ' at the WCB Forms Submission portal, <b>wcb.ny.gov</b> → “File a Claim / Submit Forms.”'));
    ...
    setTimeout(function () {
      initSig(sigCanvas);
      var c33Link = $('c3fw-c33-only-link');
